...
def run():
    ...
    #create_traces(net)
    alias = create_alias()
    topo = create_virtual_topo(alias, net)
    net.stop()
...
def compute_distances(net):
    ...
    for h1 in hosts:
        for h2 in hosts:
            if h1 != h2:
                print "Starting ping from " + h1.name + " ... "
                host1 = h1.name
                host2 = h2.name
                # Ping only between pairs of hosts (discard routers)
                if host1[0:1]== 'h' and host2[0:1] == 'h': 
                ...
                # and the distance from host1 (exploit TTL to derive the distance)
                    os.system("echo -n '" + host2 + " '  >> distances/" + host1)
...
def create_traces(net):
    hosts = net.hosts
    for h1 in hosts:
        print "Starting collection of traces from " + h1.name + " ... "
        for h2 in hosts:
            if h1 != h2:
                host1 = h1.name
                host2 = h2.name
                # Traceroute only between pairs of hosts (discard routers)
                if host1[0:1]== 'h' and host2[0:1] == 'h': 
                    h1.cmd('traceroute -n -w 0.5 ' + h2.IP() + ' > traceroute/' + h1.name + h2.name)
...
def add_routers(topo,host1,host2,x):
    with open("traceroute/"+host1+host2) as trace:
        ...
        for i in range(1, num_lines-2):  #skip first and last line (don't consider hosts)
            src = find_router(lines[i].split())
            dst = find_router(lines[i+1].split())
